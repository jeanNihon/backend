...
app.post('/api/areas', async (req, res) => {
    ...
    if (!nombre_area)
        return res.status(400).json({ error: 'Nombre de área requerido' });
    try {
        const result = await pool.query('INSERT INTO inventario_areas (nombre_area) VALUES ($1) RETURNING *', [nombre_area.toUpperCase().trim()]);
...
app.post('/api/metadata', async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO inventario_metadata (nombre_campo, etiqueta, tipo_dato) VALUES ($1, $2, $3) RETURNING *', [nombre_campo.toLowerCase().replace(/\s+/g, '_'), etiqueta, tipo_dato || 'texto']);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/items', async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO inventario_items 
            (area_inventario, tienda, usuario, dispositivo, marca, modelo, serie, placa_sinsa, campos_dinamicos) 
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`, [area_inventario, tienda, usuario, dispositivo, marca, modelo, serie, placa_sinsa, campos_dinamicos || {}]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.put('/api/items/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE inventario_items SET 
            ...
            WHERE id = $9 RETURNING *`, [tienda, usuario, dispositivo, marca, modelo, serie, placa_sinsa, campos_dinamicos, id]);
